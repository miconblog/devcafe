...
4. /flux/base.jsx 파일에 사용할 스토어 추가하기 (for CSR)
5. /server/render-react.js 파일에 스토어 추가하기 (for SSR)
5. 사용할 컴포넌트에 flux 주입하기, 최상위 컴포넌트부터 하위 컴포넌트까지 차례로 주입한다.
6. 상위 컴포넌트에 스토어를 두고 연결할것인지 모든 하위 컴포넌트에 스토어를 다 맵핑할것인지 결정해서 스토어의 위치를 결정!
   동적으로 바뀌는 댓글같은 부분 컴포넌트는 중간 상위 컴포넌트에 스토어 위치를 결정하고 나머지는 상태만 전이시키는게 유리하다.
